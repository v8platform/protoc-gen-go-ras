...
func (r rasGenerator) generateEndpointMessageHelpers(m *protogen.Message, ext MessageExtension) {
	...
	r.g.P("}")
	r.g.P()
	r.g.P("func New", m.GoIdent.GoName, "(endpoint Endpoint, message ", messageFormatterName, ") (*", m.GoIdent, ", error){")
	...
	r.g.P("}")
	r.g.P()
	r.g.P("func (x *", m.GoIdent, ") Unpack(endpoint Endpoint, into ", messageParserName, ") error {")
...
func (r rasGenerator) generateEndpointHelpers(m *protogen.Message, ext MessageExtension) {
	...
	endpointImplName := "Endpoint"
	messageName := m.GoIdent
	messageFormatterName := messageName.GoName + "Formatter"
	messageParserName := messageName.GoName + "Parser"
	...
	g.P("}")
	g.P()
	g.P("type Channel interface {")
	g.P("SendMsg(ctx ", ctxPackage.Ident("Context"), ", msg interface{}, opts ...interface{}) error")
	g.P("RecvMsg(ctx ", ctxPackage.Ident("Context"), ", msg interface{}, opts ...interface{}) error")
	g.P("}")
	g.P()

	g.P("func EndpointChannelRequest(ctx ", ctxPackage.Ident("Context"), ", channel Channel, endpoint Endpoint, req ", messageFormatterName, ", reply ", messageParserName, ") error {")
	g.P("")
	g.P("reqPacket, err := NewEndpointMessage(endpoint, req)")
	g.P("if err != nil {")
	g.P("return err")
	g.P("}")
	g.P("")
	g.P("replyPacket := &", messageName, "{}")
	g.P("")
	g.P("if err = PacketChannelRequest(ctx, channel, reqPacket, replyPacket); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("")
	g.P("return replyPacket.Unpack(endpoint, reply)")
	g.P("}")
...
func (r rasGenerator) generatePacketHelpers(m *protogen.Message, ext MessageExtension) {
	...
	g.P("}")
	g.P()
	g.P("func getPacket() *Packet {")
	g.P("return packetPool.Get().(*Packet)")
	g.P("}")
	g.P()
	g.P("func putPacket(packet *Packet) {")
	g.P("")
	g.P("packet.Type = 0")
	g.P("packet.Size = 0")
	g.P("packet.Data = []byte{}")
	g.P("packetPool.Put(packet)")
	g.P("}")
	g.P()
	g.P("var packetPool = ", syncPackage.Ident("Pool"), "{New: func() interface{} {")
	g.P("	return &Packet{}")
	g.P("}}")
	g.P()
	g.P("func RecvPacketMsg(ctx ", ctxPackage.Ident("Context"), ", channel Channel, packetMessage ", messageParserName, ") error {")
	g.P("")
	g.P("packet := getPacket()")
	g.P("defer putPacket(packet)")
	g.P("")
	g.P("if err := channel.RecvMsg(ctx, packet); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("if err := packet.Unpack(packetMessage); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("")
	g.P("return nil")
	...
	g.P("}")
	g.P()
	g.P("func SendPacketMsg(ctx ", ctxPackage.Ident("Context"), ", channel Channel, packetMessage ", messageFormatterName, ") error {")
	g.P("")
	g.P("buf := &", bytesPackage.Ident("Buffer"), "{}")
	g.P("")
	g.P("if err := packetMessage.Formatter(buf, 0); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("")
	g.P("if packetMessage.GetPacketType() == PacketType_PACKET_TYPE_NEGOTIATE {")
	g.P("if err := channel.SendMsg(ctx, buf); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("return nil")
	g.P("}")
	...
	g.P("defer putPacket(packet)")
	g.P("")
	g.P("packet.Type = packetMessage.GetPacketType()")
	g.P("packet.Data = buf.Bytes()")
	g.P("packet.Size = int32(len(packet.Data))")
	g.P("")
	g.P("if err := channel.SendMsg(ctx, packet); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("return nil")
	g.P("")
	...
	g.P("func PacketChannelRequest(ctx ", ctxPackage.Ident("Context"), ", channel Channel, req ", messageFormatterName, ", reply ", messageParserName, ") error {")
	g.P("")
	g.P("if err := SendPacketMsg(ctx, channel, req); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("")
	g.P("if reply == nil {")
	...
	g.P("}")
	g.P("")
	g.P("if err := RecvPacketMsg(ctx, channel, reply); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("return nil")
	g.P("}")
...
func (r rasGenerator) generateWriteToFunc(message *protogen.Message) {
	...
	r.g.P("if err := x.", r.formatFuncName, "(w, 0); err != nil { return 0, err }")
	r.g.P("return int64(0), nil")
	r.g.P("}")
	r.g.P()
	r.g.P("func (x *", message.GoIdent, ") ReadFrom (r ", ioPackage.Ident("Reader"), ") (int64, error) {")
	r.g.P("if err := x.", r.parseFuncName, "(r, 0); err != nil { return 0, err }")
	r.g.P("return int64(0), nil")
	r.g.P("}")
	r.g.Unskip()
