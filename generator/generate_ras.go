...
func (r rasGenerator) generateEndpointHelpers(m *protogen.Message, ext MessageExtension) {
	...
	g.P("func EndpointRequestHandler(endpoint ", endpointImplName, ", req ", messageFormatterName, ", reply ", messageParserName, ") func(ctx ", ctxPackage.Ident("Context"), ", rw ", ioPackage.Ident("ReadWriter"), ") error {")
	g.P("return func(ctx ", ctxPackage.Ident("Context"), ", rw ", ioPackage.Ident("ReadWriter"), ") error {")
	g.P("")
	...
	g.P("")
	g.P("if err = PacketRequestHandler(reqPacket, replyPacket)(ctx, rw); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("")
...
func (r rasGenerator) generatePacketHelpers(m *protogen.Message, ext MessageExtension) {
	...
	g.P()
	g.P("func RecvPacketMsg(reader ", ioPackage.Ident("Reader"), ", packetMessage ", messageParserName, ") error {")
	g.P("")
	g.P("packet := getPacket()")
	g.P("defer putPacket(packet)")
	g.P("")
	g.P("if err := packet.Parse(reader, 0); err != nil {")
	g.P("return err")
	g.P("}")
	...
	g.P("func PacketRequestHandler(req ", messageFormatterName, ", reply ", messageParserName, ") func(ctx ", ctxPackage.Ident("Context"), ", rw ", ioPackage.Ident("ReadWriter"), ") error {")
	g.P("return func(ctx ", ctxPackage.Ident("Context"), ", rw ", ioPackage.Ident("ReadWriter"), ") error {")
	g.P("")
	g.P("if err := SendPacketMsg(rw, req); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("")
	g.P("if reply == nil {")
	...
	g.P("}")
	g.P("")
	g.P("if err := RecvPacketMsg(rw, reply); err != nil {")
	g.P("return err")
	g.P("}")
	g.P("return nil")
	g.P("}")
	...
	g.P("func SendPacketMsg(writer ", ioPackage.Ident("Writer"), ", packetMessage ", messageFormatterName, ") error {")
	g.P("")
	...
	g.P("")
	g.P("_, err := packet.WriteTo(writer)")
	g.P("if err != nil {")
	g.P("return err")
	g.P("}")
	g.P("return nil")
